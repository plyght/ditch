...
pub const Study = struct {
    ...
    pub fn reset(self: *Study, settings_json: []const u8) !void {
        ...
        self.trials.clearRetainingCapacity();
        self.finished = false;
        self.settings_json = try self.arena.allocator().dupe(u8, settings_json);
    ...
    /// Indices of Pareto-optimal trials, sorted by their losses.
    pub fn bestTrials(self: *const Study, gpa: Allocator) ![]usize {
        const n = self.trials.items.len;
        const losses = try gpa.alloc([]const f64, n);
        defer gpa.free(losses);
        for (self.trials.items, 0..) |t, i| losses[i] = t.losses;
        const ranks = try tpe.nonDominatedRanks(gpa, losses);
        defer gpa.free(ranks);
        var best = std.ArrayList(usize).empty;
        for (ranks, 0..) |r, i| if (r == 0) try best.append(gpa, i);
        std.mem.sort(usize, best.items, losses, struct {
...
test "study round trip" {
    ...
    try std.testing.expectEqual(@as(usize, 3), reloaded.trials.items.len);
