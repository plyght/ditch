...
pub const Slot = enum { q, k, v, qkv, o, gate, up, gate_up, down, router, q_a, q_b, kv_a, kv_b, lin_qkvz, lin_qkv, lin_z, lin_b, lin_a, lin_ba, lin_conv, lin_q, lin_k, lin_v, lin_conv_q, lin_conv_k, lin_conv_v, lin_f_a, lin_f_b, lin_g_a, lin_g_b, light_qkv, light_gate, ssm_in, ssm_x, ssm_dt, ssm_out };
...
/// Recurrent state of the linear-attention (Gated DeltaNet, Qwen hybrids)
/// or Mamba layers: per (recurrent layer, batch slot) an f32 state
/// (`[v_heads][k_dim][v_dim]`, `[heads][head_dim][state]` or
/// `[inter][state]`) plus the causal-convolution history, with the next
/// expected position. Slots reset when a sequence restarts at position 0,
/// so caches stay reusable across batch operations like the KV cache;
/// anything else discontinuous is an error rather than a silent wrong result.
pub const LinearCache = struct {
    gpa: Allocator,
    batch: usize,
    /// Per model layer: recurrent-layer index or null.
    lin_index: []?u32,
    state_len: usize,
    conv_len: usize,
    buf: []f32,
    ...
    const Lens = struct { state: usize, conv: usize };

    /// State and convolution-history floats per (layer, slot).
    fn lens(c: *const Config) Lens {
        if (c.has_linear) return .{ .state = linearStateLen(c), .conv = linearConvLen(c) };
        const d = &c.ssm;
        return switch (d.kind) {
            .mamba2 => .{ .state = d.heads * d.head_dim * d.state, .conv = ssmConvDim(c) * (d.conv_kernel - 1) },
            .mamba1 => .{ .state = d.inter * d.state, .conv = d.inter * (d.conv_kernel - 1) },
            .none => .{ .state = 0, .conv = 0 },
        };
    }

    fn isRecurrent(c: *const Config, li: usize) bool {
        return c.linear_layers[li] or c.ssm_layers[li];
    }

    pub fn bytesFor(c: *const Config, batch: usize) u64 {
        if (!c.hasRecurrent()) return 0;
        const l = lens(c);
        const per: u64 = @intCast(l.state + l.conv);
        var n_lin: u64 = 0;
        for (0..c.num_layers) |li| {
            if (isRecurrent(c, li)) n_lin += 1;
        }
    ...
    pub fn init(gpa: Allocator, c: *const Config, batch: usize) !LinearCache {
        ...
        const l = lens(c);
        const state_len: usize = l.state;
        const conv_len: usize = l.conv;
...
///
/// The "attention" half may be a Gated DeltaNet or Mamba block instead, or
/// (Falcon-H1) a Mamba block and attention side by side on the same input
/// whose outputs are summed; single-block layers (Mamba2, Nemotron-H) run
/// only one of the two halves behind the layer norm.
fn layerBlock(model: *const Model, layer: *const Layer, li: usize, ws: *Workspace, cache: *KvCache, x: []f32, rows: []const Row) !void {
    ...
    const has_mixer = has_attn or has_ssm or c.linear_layers[li];
