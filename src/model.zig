...
pub const Slot = enum { q, k, v, qkv, o, gate, up, gate_up, down, router, q_a, q_b, kv_a, kv_b, lin_qkvz, lin_qkv, lin_z, lin_b, lin_a, lin_ba, lin_conv, light_qkv, light_gate };
...
/// `qkv`/`z`/`b`/`a` set (Qwen3.5) plus the convolution and time-step
/// vectors; MiniMax lightning attention carries `light_qkv`, `light_gate`
/// and `norm`. The output projection lives in `Layer.o` so that abliteration
/// treats it like any attention output.
pub const LinearWeights = struct {
    ...
    dt_bias: []const f32 = &.{},
    a_log: []const f32 = &.{},
    /// Gated RMSNorm weight `[v_dim]` (DeltaNet) or `[heads * head_dim]` (lightning).
    norm: []const f32 = &.{},
...
pub const Model = struct {
    ...
    fn loadLinear(self: *Model, layer: *Layer, arena: Allocator, li: usize, lp: []const u8) !void {
        if (self.config.linear_kind == .lightning) return self.loadLightning(layer, arena, li, lp);
        const c = &self.config;
...
fn linearStateLen(c: *const Config) usize {
    return switch (c.linear_kind) {
        .gdn => c.linear_v_heads * c.linear_k_dim * c.linear_v_dim,
...
fn linearConvLen(c: *const Config) usize {
    return switch (c.linear_kind) {
        .gdn => linearConvDim(c) * (c.linear_conv_kernel - 1),
        .lightning => 0,
    };
}
...
pub const Workspace = struct {
    ...
    fn linearCols(c: *const Config) usize {
        if (!c.has_linear) return 0;
        return switch (c.linear_kind) {
            .gdn => 2 * c.linear_k_heads * c.linear_k_dim + 3 * c.linear_v_heads * c.linear_v_dim + 2 * c.linear_v_heads,
...
fn linearForward(model: *const Model, layer: *const Layer, li: usize, ws: *Workspace, cache: *KvCache, h: []const f32, rows: []const Row) !void {
    const c = &model.config;
    if (c.linear_kind == .lightning) return lightningForward(model, layer, li, ws, cache, h, rows);
    const gpa = model.gpa;
