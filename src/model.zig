...
pub const Slot = enum { q, k, v, qkv, o, gate, up, gate_up, down, router, q_a, q_b, kv_a, kv_b, lin_qkvz, lin_qkv, lin_z, lin_b, lin_a, lin_ba, lin_conv, conv_in, conv_kernel, ple_gate, ple_out, laurel_l, laurel_r, altup_router, altup_predict, altup_correct };
...
pub const Model = struct {
    ...
    fn loadWeights(self: *Model) !void {
        ...
        for (self.layers, 0..) |*layer, i| {
            ...
            if (c.linear_layers[i]) {
                ...
                const qwant = if (c.gated_attention) 2 * c.num_heads * hd else c.num_heads * hd;
                if (layer.q.?.rows != qwant or layer.q.?.cols != c.hidden_size) {
                    std.log.err("layer {d}: q projection is [{d}][{d}], expected [{d}][{d}] (heads {d}, head_dim {d})", .{ i, layer.q.?.rows, layer.q.?.cols, qwant, c.hidden_size, c.num_heads, hd });
                    return error.InvalidConfig;
                }
            ...
            if (!c.linear_layers[i] and !c.conv_layers[i]) {
                ...
                const owant = c.num_heads * c.layerVDim(i);
    ...
    fn buildRope(self: *Model) !void {
        const c = &self.config;
        const len = @min(c.max_position_embeddings, 8192);
...
/// tokens: Gated DeltaNet linear-attention layers (Qwen hybrids: a
/// `[v_heads][k_dim][v_dim]` f32 state plus the causal-convolution history)
/// and LFM2 short-convolution layers (the `[hidden][taps - 1]` history). One
...
pub const LinearCache = struct {
    ...
    fn slotLenOf(c: *const Config, li: usize) usize {
        if (c.linear_layers[li]) return c.linear_v_heads * c.linear_k_dim * c.linear_v_dim + linearConvDim(c) * (c.linear_conv_kernel - 1);
    ...
    pub fn init(gpa: Allocator, c: *const Config, batch: usize) !LinearCache {
        ...
        return .{
            ...
            .offsets = offsets,
            .lens = lens,
            .state_len = if (c.has_linear) c.linear_v_heads * c.linear_k_dim * c.linear_v_dim else 0,
            .conv_len = if (c.has_linear) linearConvDim(c) * (c.linear_conv_kernel - 1) else 0,
            .buf = buf,
            .next_pos = next_pos,
        };
    }
...
pub const Workspace = struct {
    ...
    pub fn bytesPerRow(c: *const Config) u64 {
        ...
        return (5 * hidden + 2 * qd + 2 * kvd + fusedQkvRows(c) + 2 * inter + fusedGateUpCols(c) + linearCols(c) + altCols(c) + pleCols(c)) * 4;
    }
...
pub fn forward(model: *const Model, ws: *Workspace, cache: *KvCache, tokens: []const u32, rows: []const Row, opts: ForwardOptions) !void {
    ...
    if (!single) {
        act = try stream.Activations.init(gpa, model.io, n, hidden, act_opts);
    ...
    for (0..c.num_layers) |li| {
        ...
        while (start < n) : (start += chunk_rows) {
            ...
            if (!single) {
                try act.?.commit(start, cn, xs);
                if (alt_w > 0) try act_alt.?.commit(start, cn, alts);
            }
    ...
    if (opts.logit_rows.len > 0) {
        ...
        for (opts.logit_rows, 0..) |r, i| {
            ...
            if (single) {
                src = ws.x[r * hidden ..][0..hidden];
            } else {
                try act.?.readRow(r, ws.o[0..hidden]);
                src = ws.o[0..hidden];
            }
            if (alt_w > 0) {
                ...
                try altupFold(model, unembed[0..n_unembed], src, alts, ws.h2[0..hidden], ws.h[0..hidden]);
                applyNorm(c, dst, ws.h2[0..hidden], model.final_norm);
            } else {
...
/// The sequence-mixing sublayer of a layer: attention, Gated DeltaNet or a
/// short convolution. Reads `h`, writes `ws.o`.
fn mixer(model: *const Model, layer: *const Layer, li: usize, ws: *Workspace, cache: *KvCache, h: []const f32, rows: []const Row) !void {
    const c = &model.config;
    if (c.linear_layers[li]) return linearForward(model, layer, li, ws, cache, h, rows);
