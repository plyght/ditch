...
pub const Settings = struct {
    ...
    /// Seconds a request of the remote source may keep failing transiently
    /// (network down, 5xx, 429) before the run ends; null retries until the
    /// network comes back (Ctrl+C stops).
    remote_retry_timeout_seconds: ?u64 = null,
...
pub const help_sections = [_]HelpSection{
    ...
    \\  --remote-retry-timeout <time>  End the run when the network stays down this long
    \\                                 (default: wait until it comes back; Ctrl+C stops).
