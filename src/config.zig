...
pub fn load(gpa: Allocator, io: std.Io, args: []const []const u8, environ: ?*std.process.Environ.Map) !LoadResult {
    ...
    if (environ) |env| {
        const vars = [_][2][]const u8{ .{ "DITCH_THREADS", "threads" }, .{ "DITCH_MAX_RAM", "max_ram" }, .{ "DITCH_DEVICE", "device" }, .{ "DITCH_GPU_MEMORY", "gpu_memory" }, .{ "DITCH_DEVICE_MIN_MACS", "device_min_macs" } };
