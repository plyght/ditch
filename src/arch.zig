            ...
            for (lt.array.items, 0..) |v, i| {
                if (i < layers and v == .string) {
                    ...
                    if (std.mem.eql(u8, t, "sliding_attention") or std.mem.eql(u8, t, "chunked_attention")) {
                        ...
                        conv_layers[i] = true;
                        has_conv = true;
                    } else if (std.mem.eql(u8, t, "full_attention") or std.mem.eql(u8, t, "attention") or std.mem.eql(u8, t, "indexed_attention")) {
                        // `indexed_attention` (Qwen4-Exp, GLM-5.3-Flash): full
                        // attention behind a sparse indexer whose dense
                        // equivalent runs within `Config.index_bound`.
