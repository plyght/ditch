...
pub const Config = struct {
    ...
    /// Multiplier on the value vectors before attention (MiMo V2 `attention_value_scale`).
    value_scale: f32,
    /// Sliding-window layers rotate with `rope_local_theta` instead of `rope_theta`.
    sliding_rope_local: bool,
    /// Per layer: key/value heads. `num_kv_heads` is the largest of them and
    /// the KV cache stride; families with one value have it on every layer.
...
pub fn parseConfig(arena: Allocator, json_text: []const u8) !Config {
    ...
    // Filled by the family hook for families with per-layer kv heads; 0 =
    // the model-wide `num_kv_heads` (resolved after the hook).
    ...
    for (c.layer_kv_heads) |*k| {
        if (k.* == 0) k.* = c.num_kv_heads;
        if (k.* == 0 or c.num_heads % k.* != 0) return error.InvalidConfig;
        c.num_kv_heads = @max(c.num_kv_heads, k.*);
    }
...
fn extraGemma(c: *Config, _: Allocator, obj: std.json.ObjectMap) !void {
    ...
    // Gemma 3 local layers rotate with `rope_local_base_freq`.
    if (std.mem.startsWith(u8, c.model_type, "gemma3")) c.sliding_rope_local = true;
}
...
fn extraMiMoV2(c: *Config, arena: Allocator, obj: std.json.ObjectMap) !void {
    ...
    c.value_scale = if (obj.get("attention_value_scale")) |v| (if (v == .null) 1.0 else getF32(obj, "attention_value_scale", 1.0)) else if (hf_spelling) 0.707 else 1.0;
    ...
    c.rope_local_theta = getF32(obj, "swa_rope_theta", if (hf_spelling) 10_000.0 else c.rope_theta);
    if (getObj(obj, "rope_parameters")) |rp| {
        ...
        if (swa) |s| {
            c.rope_local_theta = getF32(s, "rope_theta", 10_000.0);
...
test "parseConfig handles both MiMo V2 spellings" {
    ...
    try std.testing.expectEqual(@as(usize, 4), hf.num_kv_heads);
    try std.testing.expectEqual(@as(usize, 16), hf.v_head_dim);
    ...
    try std.testing.expectEqual(@as(f32, 10_000.0), hf.rope_local_theta);
    try std.testing.expect(hf.sliding_rope_local and hf.sinks and hf.sinks_sliding_only);
    try std.testing.expectEqual(@as(f32, 0.707), hf.value_scale);
    ...
    try std.testing.expectEqual(@as(f32, 1.0), hub.value_scale);
