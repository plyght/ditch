...
Nemotron-H, RWKV), Kimi K3 / K2.5+ (gated linear attention, no
`tokenizer.json`), Kimi K2 (no `tokenizer.json`), Qwen3.8-Flash-Next
(`qwen4_exp`), GLM-5.3-Flash (`glm5_next`) and DeepSeek V4 (sparse indexers
with hyper-connections and hash layers), encoder-decoder models, Gemma 3n
(per-layer inputs), MiniCPM3, GraniteMoE, OPT-350m (projection layers),
quantisation formats other than the ones listed below (GPTQ, AWQ,
bitsandbytes, ...), and tokenizers without a `tokenizer.json`
