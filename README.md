...
compressed-tensors weights), Kimi K2 (FP8 weights), Qwen3.8-Flash-Next
(`qwen4_exp`), GLM-5.3-Flash (`glm5_next`) and DeepSeek V4 (sparse indexers
with hyper-connections and hash layers, FP4/FP8 weights), encoder-decoder
models, Gemma 3n (per-layer inputs), MiniCPM3, GraniteMoE, OPT-350m
(projection layers), FP8 checkpoints, and SentencePiece-only tokenizers
(Baichuan; generate a `tokenizer.json` with
