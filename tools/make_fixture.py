...
Usage: make_fixture.py <family> [<out_dir>] [--gguf]
    family: llama | qwen2 | qwen3 | gemma3 | qwen3_moe | qwen3_moe_fused | qwen3_moe_fused_t
            ...
            the Mamba families mamba2, nemotron_h, falcon_h1, jamba,
            granitemoehybrid and their variants)
            | qwen3_moe_big
...
spec("granitemoehybrid", tok="starcoder", L=3, pos="none", embed_scale=2.0, attn_scale=0.25, residual_mult=0.5, logit_scale=0.25, lm_head=None,
     ...
     moe={"E": 4, "K": 2, "MI": 12, "shared": 1, "shared_inter": 16, "shared_fused": True, "scoring": "softmax", "group_limited": False,
          "rsf": 1.0, "norm": True, "layers": [0, 1, 2], "corr_bias": False, "layout": "fused_rows", "prefix": "block_sparse_moe.",
          "router": "router.layer.weight", "fused_names": ("input_linear.weight", "output_linear.weight"), "shared_name": "shared_mlp."},
    ...
    for i in range(L):
        ...
        if moe and i in moe["layers"] and mlp_layers[i]:
            ...
            if moe["layout"] == "separate":
                ...
                # [E, 2I, H] and [E, H, I] (Granite `input_linear` / `output_linear`).
                gu_name, dn_name = moe.get("fused_names", ("experts.gate_up_proj", "experts.down_proj"))
                weights[mp + gu_name] = np.stack([np.concatenate([ex["gate"], ex["up"]], 0) for ex in experts])
                weights[mp + dn_name] = np.stack([ex["down"] for ex in experts])
            else:
            ...
            if moe["shared"]:
                ...
                if moe.get("shared_fused"):
                    # Granite: `shared_mlp.input_linear` holds gate rows then up rows.
                    sp = lp + moe["shared_name"]
                    gu = mat(sp + "input_linear.weight", 2 * si, H)
                    d["shared"] = {"gate": gu[:si], "up": gu[si:], "down": mat(sp + "output_linear.weight", H, si), "gate_vec": None}
                elif moe.get("dense"):
