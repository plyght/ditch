"""truncate_checkpoint.py REPO N [OUT_DIR] [--drop PREFIX]... [--rows NAME=FILE]...

Build a checkpoint of the first N decoder layers by range-fetching only the
needed tensors from the remote safetensors shards: each shard's header first,
then the embedding, the final norm, the LM head and layers 0..N-1, streamed
straight into one model.safetensors (nothing is held in memory, no full shard
is downloaded).

config.json gets num_hidden_layers = N and every per-layer list cut to N
(layer_types, mlp_layer_types, compress_ratios, ...); per-layer *id* lists
(kv_source_layer_ids, engram_layer_ids, ...) keep the ids below N, and a list
paired with one of them (engram_num_embeddings) keeps the matching entries.
Quantisation and everything else stays exactly as released.
...
--rows NAME=FILE
                write tensor NAME as a sparse file region holding only the rows
                listed in FILE (one row index per line): the rest of the tensor
                is a hole that takes no disk space and reads as zeros. For
                tables far larger than the disk (DeepSeek V4.1's engram
                embeddings) of which a prompt reads a handful of rows. A
                sibling scale tensor `<module>.scale` with the same row count is
                cut the same way.
"""
...
while i < len(args):
    if args[i] == '--drop':
        ...
        k, v = args[i + 1].split('=', 1); rows_spec[k] = v; i += 2
    else:
        pos.append(args[i]); i += 1
repo, N = pos[0], int(pos[1])
out = pos[2] if len(pos) > 2 else f"models/{repo.replace('/','__')}-L{N}"
...
# Pass 1: headers.
plan = []  # (name, dtype, shape, url, abs_start, nbytes)
for sh in shards:
    ...
    for k, v in sorted(hdr.items(), key=lambda kv: kv[1]['data_offsets'][0]):
        ...
        a, b = v['data_offsets']
        plan.append((k, v['dtype'], v['shape'], url, 8 + n + a, b - a))

# Sparse rows: the tensor named by --rows and its `.scale` sibling.
sparse = {}
for name, path in rows_spec.items():
    rows = sorted(set(int(x) for x in open(path).read().split()))
    sparse[name] = rows
    if name.endswith('.weight'):
        sparse[name[:-len('.weight')] + '.scale'] = rows
...
for k, dt, shp, url, start, nb in plan:
    ...
    if k in sparse:
        row_bytes = nb // shp[0]
        for r in sparse[k]:
            jobs.append((url, start + r * row_bytes, row_bytes, dst + r * row_bytes))
        continue
    done = 0
    while done < nb:
        m = min(CH, nb - done)
...
with ThreadPoolExecutor(16) as ex:
    for i, m in enumerate(ex.map(fetch, jobs)):
        fetched += m
        if i % 50 == 0: print(f'{fetched/1e9:.2f} GB of {sum(j[2] for j in jobs)/1e9:.2f}', flush=True)
os.close(fd)
print(out, len(plan), 'tensors', off / 1e9, 'GB logical,', fetched / 1e9, 'GB fetched')
